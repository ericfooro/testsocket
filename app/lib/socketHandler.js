...
var uri = 'ws://192.168.43.34:9000/socket2/server.php';
...
var socketHandler = function(args) {
			...
			if (_enableReconnecting) {
				...
				if (!_isReconnecting) {
					...
					websocket.addEventListener('message', function(ev) {
						...
						Ti.API.info('[SOCKET - message status] :: ' + JSON.stringify(ev));
		...
		Ti.App.addEventListener("connected", function(event) {
			Ti.API.info('[SOCKET] :: Socket connected ');
...
socketHandler.prototype.checkMessagingStatus = function(event) {
	Ti.API.info('[SOCKET - message status] :: ' + JSON.stringify(event));
...
socketHandler.prototype.connect = function() {
	Ti.API.info('[SOCKET] :: Socket opening ');
	websocket.open(uri);//argu["URL"]
	...
	websocket.addEventListener('message', function(ev) {
		...
		Ti.API.info('[SOCKET] :: Socket _lastReceivedMessage ' + _lastReceivedMessage);
		Ti.API.info('[SOCKET - message] :: ' + JSON.stringify(ev));
		Ti.App.fireEvent("socketMessage", {
			event: ev
		});
	});
	
	websocket.addEventListener('onmessage', function(ev) {
		// ev.data contains message data
		_lastReceivedMessage = Math.round(new Date().getTime() / 1000);
		Ti.API.info('[SOCKET] :: Socket _lastReceivedMessage ' + _lastReceivedMessage);
		Ti.API.info('[SOCKET - message] :: ' + JSON.stringify(ev));
